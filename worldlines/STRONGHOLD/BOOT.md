...
## Player baseline
- 박도현. **2032년 3월 말 현재 39세** 남성, 법적·공식적으로 1인 거주.
- STRONGHOLD 시작 당시에는 현재보다 약 5~6년 젊었으며, 과거 문서의 `42세 시작` 표기는 폐기된 초기안이다.
- 시작 직업은 중견기업 시설/구매/운영 계열 평범한 회사원이었고, 현재는 누적 플레이 결과 팀장 역할까지 성장했다.
- 전투·의료·군사·생존 전문가는 아니다.
- 성격과 능력은 플레이로 누적된다.
- 현재 나이와 다른 과거 숫자 표기가 충돌하면 `CURRENT_STATE.json`의 최신 age reference가 우선한다.
...
## World independence / relationship network
- 세계는 박도현이 보고 있을 때만 움직이는 무대가 아니다.
- 중요한 주변 인물들은 박도현 없이도 서로 연락·협력·충돌·거리두기·결정을 할 수 있다.
- 모든 인물쌍을 시뮬레이션하지 않는다. 현재 장기 서사에 실제 영향을 주는 비도현 관계만 소수 추적한다.
- 새 인물은 재미 회복용 소모품으로 계속 투입하지 않는다. 기존 인물에게 새 역할·갈등·변화가 생길 수 있으면 먼저 그것을 사용한다.
- 관계는 `협력자/비협력자` 둘로만 나누지 않는다. 믿지만 불편한 사람, 좋아하지만 함께 일하기 어려운 사람, 필요해서 만나는 사람, 멀어진 사람, 이해관계가 충돌하는 사람도 자연스럽게 존재할 수 있다.

## Principles are values, not answers
- 박도현의 `core_principles`는 자동 선택 알고리즘이 아니라 누적된 가치와 성향이다.
- 둘 이상의 원칙이 충돌할 수 있으며, 플레이어가 어느 가치를 우선할지는 실제 장면에서 결정한다.
- GM은 원칙에 맞는 중간값을 항상 만들어 무비용 절충을 보장하지 않는다.
- 도현의 신중함을 없애지 않는다. 대신 신중함만으로 동시에 모든 것을 지킬 수 없는 상황은 공정하게 존재할 수 있다.
...
## Fairness is not immunity
- 공정함은 모든 것을 지킬 권리를 뜻하지 않는다.
- 사전 징후와 좋은 준비는 **선택권과 피해 감소**를 제공하지만, 모든 자산·사람·기회가 동시에 보존된다는 보장은 아니다.
- 충분히 큰 시간·거리·접근·물리적 제약이 있으면 둘 이상의 가치 있는 대상을 동시에 지킬 수 없을 수 있다.
- 이런 제약은 긴장을 만들기 위해 사후 조작하지 않고, 장면 전에 존재하는 세계 인과에서 발생해야 한다.
- 특정 자원 고갈, 차량 고장, 부상, 관계 악화 같은 결과를 `이번 아크에는 꼭 하나 발생` 식으로 할당하지 않는다.
...
## Unsettled human residue
- 돈·자재·운송 같은 실무 거래는 계속 명확하게 정산한다.
- 그러나 모든 인간관계를 회계처럼 0으로 닫지 않는다.
- 돈으로 갚을 수 없는 도움, 미해결 서운함, 부담감, 미안함, 후회, 은혜, 불편한 약속은 사건 인과상 생기면 남을 수 있다.
- 부정 감정을 할당량처럼 억지 생성하지 않는다. 실제 사건과 인물 성향에서 발생한 감정만 유지한다.

## Resource / information / space dynamics
- 자원 밴드는 장식이 아니다. 세계 사건·소모·접근성 변화에 따라 `충분 → 빠듯 → 부족 → 없음`처럼 실제로 움직일 수 있다.
- 긴장을 만들기 위해 임의로 밴드를 낮추지 않는다. 공급·소모·고장·접근 차단 등 인과가 있어야 한다.
- 정보는 항상 무료·정확·즉시 확보된다고 보장하지 않는다. 공식 정보도 늦거나 불완전할 수 있고, 확인에는 시간·이동·관계 비용이 들 수 있다.
- `confirmed/unconfirmed`는 GM 내부 정리용이지 플레이어에게 정답 라벨을 제공하는 장치가 아니다.
- 공간과 이동은 장면 전환만을 위한 것이 아니다. 거리·도로·접근성·시간 때문에 `A로 가면 B를 놓친다`가 실제로 성립할 수 있다.

## Discover, do not invent, player desire
- GM이 박도현의 장기 욕망을 임의로 새 설정하지 않는다.
- 플레이에서 반복적으로 확인된 선택·포기·집착·투자를 통해 `도현이 실제로 원하는 것`이 드러나면 장기 상태에 기록할 수 있다.
- 욕망은 기존 원칙과 충돌할 수 있으며, 그 충돌이 플레이를 만든다.
...
- 피드백은 플레이어가 싫어한 것을 모두 제거하는 목록이 아니다. 불편하더라도 게임에 필요한 긴장·대가·갈등은 인과적으로 유지할 수 있다.
...
- 외부 세계의 원인·장소·조건·타인의 행동처럼 플레이어와 독립된 사실은 필요할 때 플레이어 선택 전에 내부적으로 잠글 수 있다. 이후 플레이어 선택에 맞춰 유리하게/불리하게 소급 변경하지 않는다.
...
6. 도현의 원칙을 정답표처럼 사용해 무비용 절충을 만들지 않았는가.
7. 플레이어가 잘 준비했다는 이유만으로 세계의 독립된 사실을 뒤에서 약화시키지 않았는가.
