...
| `SESSION_MISSING_20260925T0932KST` | current ChatGPT room identity audit | No AFTERFALL verbatim range verified; directly visible text belongs to C02 / STRONGHOLD / 박도현 | 0 archived | 0 archived | MISSING |
...
`SESSION_MISSING_20260925T0932KST` is an audit-only session marker. It contains no transcript PART because copying the directly visible C02 / STRONGHOLD text into C03 / AFTERFALL would violate identity isolation.
...
The identity-audit session has no AFTERFALL transcript content, so transcript overlap is not applicable to it.
...
- `SESSION_002`: everything before its first directly visible USER turn is
  `MISSING_TRANSCRIPT`.
- `SESSION_MISSING_20260925T0932KST`: the claimed C03 / AFTERFALL / 서진우 S02 source transcript is `[원문 확인 불가 구간]` in the current model-accessible room context. The directly visible pre-cutoff public conversation belongs to C02 / STRONGHOLD / 박도현 and is intentionally not copied here.
...
3. `SESSION_MISSING_20260925T0932KST/INDEX.md` only for the identity-audit record; no transcript PART exists
