...
function DetailArticle({
  ...
  const article = archiveArticleByNodeId[selected.id]
  const explicitTranscriptIds = new Set(article?.transcriptPartIds ?? [])
  const transcriptParts = transcriptPartsFor(activeChronicle.id)
    .filter((part) => part.status !== 'missing_transcript')
    .filter((part) => explicitTranscriptIds.size > 0
      ? explicitTranscriptIds.has(part.id)
      : part.relatedNodeIds.length === 0 || part.relatedNodeIds.includes(selected.id))
    .slice(0, 6)
  ...
  return (
    <article className="archive-detail" id="archive-detail" aria-labelledby="archive-detail-title">
      ...
      <nav className="detail-toc" aria-label={selected.label + ' 본문 목차'}><a href="#detail-basics">기본 정보</a><a href="#detail-overview">개요</a><a href="#detail-history">주요 행적</a><a href="#detail-relations">핵심 관계</a><a href="#detail-events">주요 사건</a><a href="#detail-locations">관련 장소</a><a href="#detail-sources">원문·출처</a><a href="#detail-timeline">연표</a><a href="#detail-canon">정본 요약</a></nav>
      ...
      <section className="detail-section detail-prose" id="detail-overview"><h2>개요</h2>{article?.lead?.length ? article.lead.map((paragraph, index) => <p key={index}>{paragraph}</p>) : <><p>{selected.label}{hasFinalConsonant(selected.label) ? '은' : '는'} {selected.subtitle}로 기록된 {typeLabel[selected.type]} 항목이다. {selected.summary}</p><p>이 문서는 그래프에서 보인 연결을 읽을 수 있는 공개 아카이브로 풀어 쓴 것이다. 현재 상태와 관계는 PLAYER_SAFE 범위에서만 정리하며, 원문에 없는 대사나 미래 전개는 보태지 않는다.</p></>}</section>
      <section className="detail-section" id="detail-history"><h2>주요 행적 · 기록</h2>{article?.sections?.length ? <div className="article-section-list">{article.sections.map((section) => <section className="article-section-block" key={section.id}><h3>{section.title}</h3>{section.paragraphs.map((paragraph, index) => <p key={index}>{paragraph}</p>)}{section.bullets && <ul>{section.bullets.map((bullet) => <li key={bullet}>{bullet}</li>)}</ul>}</section>)}</div> : <p className="archive-muted">현재 공개 정본에서 추가 장문 항목을 정리 중입니다.</p>}</section>
      ...
      <section className="detail-section" id="detail-timeline"><h2>연표</h2><ol className="detail-timeline">{article?.timeline?.length ? article.timeline.map((item, index) => <li key={item.date + item.title + index}><time>{item.date}</time><div><strong>{item.title}</strong><p>{item.text}</p></div></li>) : <><li><time>기록 등록</time><div><strong>{activeChronicle.label} · {archiveMeta.season}</strong><p>{selected.label}{hasFinalConsonant(selected.label) ? '이' : '가'} 공개 아카이브 항목으로 정리됐다.</p></div></li>{events.slice(0, 4).map((event) => <li key={event.id}><time>{event.subtitle}</time><div><strong>{event.label}</strong><p>{event.summary}</p></div></li>)}<li><time>최근 상태</time><div><strong>{statusFor(selected)}</strong><p>{selected.summary}</p></div></li></>}</ol></section>
