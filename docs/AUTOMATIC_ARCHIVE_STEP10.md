...
The first three transforms have local compilers. The image and delivery tail has no accepted real image or active production path. The Step 9 read-only command checks the existing compilers together; it does not bridge an uncommitted Reader candidate into graph compilation. Step 8 consumes a pinned public Visual Catalog to show pending states only. No hidden world seed, private GM record, or PLAYER_ARCHIVE-only map is an authorized input to this chain.
...
| Website visual state | Step 8 pending-state UI in Draft PR #145 | Local tests/build/Preview; pinned catalog validation | No image shown or site rollout | CODE_COMPLETE_BUT_UNPROVEN |
| Orchestration | Step 9 read-only pipeline check in Draft PR #146 | Real S02 Reader/graph/visual dry-run and unit tests | No unattended run or durable queue | IN_PROGRESS |
...
The Step 8 and 9 PR CI verdicts must be checked before treating their CI column as final. A successful synthetic test or compiled brief never counts as a model-generated image. A local accepted candidate never counts as stored or published.
...
3. Integrate a verified published-asset manifest into the website and inspect a nonproduction Preview. The current pending-state UI is not evidence of image delivery.
