...
function playerAction(request: GMProviderTurnRequest) {
  ...
  if (input.kind === 'free-action') return { kind: 'free-action', text: input.text }
...
function stringArray(value: JsonValue | undefined, max = 6): string[] {
  if (!Array.isArray(value)) return []
  return value.filter((item): item is string => typeof item === 'string').slice(-max)
}

/**
 * Small public-only brief for the story model. The engine keeps full state; the model only sees
 * what it needs to continue the current scene coherently.
 */
export function buildCompactGMBrief(request: GMProviderTurnRequest) {
  ...
  return {
    ...
    character_notes: isRecord(state.public_world.family_character_notes) ? state.public_world.family_character_notes : {},
...
function normalizeHint(value: unknown): StoryStateHint | undefined {
  ...
  if (value.kind === 'time' && Number.isInteger(value.minutes)) {
    return { kind: 'time', minutes: Math.max(0, Math.min(180, Number(value.minutes))) }
  }
  if (value.kind === 'move' && typeof value.entity === 'string' && typeof value.to === 'string' && value.to.trim()) {
    return { kind: 'move', entity: value.entity.trim(), to: value.to.trim() }
  }
  if (value.kind === 'resource' && typeof value.resource_id === 'string' && typeof value.to === 'string' && value.to.trim()) {
    return { kind: 'resource', resource_id: value.resource_id.trim(), to: value.to.trim() }
  }
  if (value.kind === 'base_capability' && typeof value.base_id === 'string' && typeof value.add === 'string' && value.add.trim()) {
    return { kind: 'base_capability', base_id: value.base_id.trim(), add: value.add.trim() }
  }
...
function compileHints(checkpoint: PublicRuntimeCheckpoint, hints: StoryStateHint[], turnNumber: number): QueuedAction[] {
  ...
  for (const hint of hints) {
    if (hint.kind === 'time') timeDelta = Math.min(180, timeDelta + hint.minutes)
    if (hint.kind === 'move') {
      if (PARTY_IDS.has(hint.entity as PartyMemberId) && state.party[hint.entity as PartyMemberId]) partyMoves.set(hint.entity, hint.to)
      else if (state.vehicles[hint.entity]) vehicleMoves.set(hint.entity, hint.to)
    }
    if (hint.kind === 'resource' && state.resources[hint.resource_id]) resourceTargets.set(hint.resource_id, hint.to)
    if (hint.kind === 'base_capability' && state.bases[hint.base_id] && !state.bases[hint.base_id].capabilities.includes(hint.add)) {
      capabilityTargets.set(`${hint.base_id}\u0000${hint.add}`, hint.add)
    }
    if (hint.kind === 'signal') signals.push(hint.text)
  }
  ...
  const baseCapabilityChanges = [...capabilityTargets.keys()].map((key) => {
    const [baseId, add] = key.split('\u0000')
    return { base_id: baseId, add }
  })
  ...
  if (signals.length > 0) {
    ...
    worldChanges.push({
      ...
      to: [...current, ...signals].slice(-6),
    })
  }

  if (timeDelta === 0 && moves.length === 0 && resourceChanges.length === 0 && baseCapabilityChanges.length === 0 && worldChanges.length === 0) return []

  const actors = [...new Set(moves
    .filter((move) => move.entity_type === 'party' && PARTY_IDS.has(move.entity_id as PartyMemberId))
    .map((move) => move.entity_id as PartyMemberId))]

  return [{
    id: `t${turnNumber}_story-state`,
    label: '스토리 상태 반영',
    actors: actors.length > 0 ? actors : ['player'],
    exclusive_resources: [],
    proposal: {
...
/** Compile the model-friendly output into the existing trusted engine proposal contract. */
