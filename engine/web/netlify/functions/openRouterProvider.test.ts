...
import { createWebMvpTestSession } from '../../src/runtime/webMvpTestSession'
import { OPENROUTER_DEEPSEEK_MODEL, OPENROUTER_MAX_ATTEMPTS, OpenRouterProvider, type OpenRouterObservabilityEvent } from './openRouterProvider'

function validProposal() {
  const followUpAction = {
    id: 'gm-follow-up', label: '다음 공개 상태 확인', actors: ['player'], exclusive_resources: [],
    proposal: { time_delta_min: 5, moves: [], resource_changes: [], base_capability_changes: [], world_changes: [] },
  }
  return {
    actions: [{
      id: 'gm-check-comms', label: '통신 상태 확인', actors: ['player'], exclusive_resources: [],
      proposal: {
        time_delta_min: 10, moves: [],
        resource_changes: [{ resource_id: 'communications', from: '불안정', to: '점검 중' }],
        base_capability_changes: [], world_changes: [],
      },
    }],
...
function completion(proposal: unknown, status = 200) {
  return new Response(JSON.stringify({
    ...
    usage: { prompt_tokens: 123, completion_tokens: 45, cost: 0.0001 },
  }), { status, headers: { 'x-openrouter-provider': 'test-upstream' } })
}
...
describe('OpenRouterProvider', () => {
  ...
  it('retries a transient network failure no more than once, then safely falls back', async () => {
    ...
    let calls = 0
    const provider = new OpenRouterProvider({
      apiKey: 'test-key', timeoutMs: 1, observe: (event) => events.push(event),
      fetchImpl: async () => { calls += 1; throw new Error('temporary offline') },
    })
    const result = await provider.proposeTurn({ input: { kind: 'free-action', text: '통신 상태를 확인한다' }, checkpoint: createWebMvpTestSession() })
  ...
  it('classifies a bounded abort as a timeout and retries a 5xx exactly once', async () => {
    ...
    const timeoutProvider = new OpenRouterProvider({
      apiKey: 'test-key', timeoutMs: 1, observe: (event) => timeoutEvents.push(event),
    ...
    let serverCalls = 0
    const serverProvider = new OpenRouterProvider({ apiKey: 'test-key', observe: () => {}, fetchImpl: async () => {
      serverCalls += 1
  ...
  it('rejects malformed JSON, unsupported response shapes, and schema mismatches before commit', async () => {
    ...
    const responses = [
      ...
      completion({ ...validProposal(), unexpected: true }),
    ]
  ...
  it('never forwards hidden seed or raw transcript fields to OpenRouter', async () => {
    let body = ''
    const checkpoint = createWebMvpTestSession() as typeof createWebMvpTestSession extends () => infer T ? T : never
    const contaminated = { ...checkpoint, public_state: { ...checkpoint.public_state, raw_transcript: 'do-not-send', hidden_seed: 'do-not-send' } }
    const provider = new OpenRouterProvider({ apiKey: 'test-key', observe: () => {}, fetchImpl: async (_url, init) => {
